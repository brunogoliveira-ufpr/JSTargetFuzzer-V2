...
void __sanitizer_cov_trace_pc_guard(duk_uint32_t *guard) {
    ...
    //void *targetMemoryList[] = {(void*)0x555556cc6d40}; 

    void *targetStart = (void*)0x555556cc6d40;
    void *targetEnd = (void*)0x555556cfa470;

    __shmem->edges[index / 8] |= 1 << (index % 8);


        if((((uintptr_t)PC < (uintptr_t)targetEnd) && (uintptr_t)PC >= (uintptr_t)targetStart))  { 

            __shmem->num_seccov_total += 1;
            __shmem->num_sec_per_edge_total += 1;


            duk_uint8_t isNewAddress = 1;
