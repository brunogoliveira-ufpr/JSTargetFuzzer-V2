...
int cov_initialize(struct cov_context *context)
{
...
#else
    // memset(context->shmem->file_access_counts, 0, sizeof(context->shmem->file_access_counts));
    ...
    close(fd);
#endif
    //context->shmem->num_sec_per_edge = 0.0;
    context->shmem->num_sec_cov_total = 0.0;
    //context->shmem->num_sec_per_edge_total = 0.0;
    return 0;
}

// void cov_record_file_access(struct cov_context* context, uint32_t file_number) {
//     if (file_number < MAX_FILES) {
//         context->file_access_counts[file_number]++;
//     } else {
//         fprintf(stderr, "[LibCoverage] File number out of bounds: %u\n", file_number);
//     }
// }
...
void cov_finish_initialization(struct cov_context *context, int should_track_edges)
{
    ...
    // memset(context->file_access_counts, 0, sizeof(context->file_access_counts));
    ...
    // Inicialize os valores de sec_cov
    context->ctx_num_sec_cov = 0.0;
    context->ctx_num_sec_cov_total = 0.0;
    //context->ctx_num_sec_per_edge = 0.0;
    //context->ctx_num_sec_per_edge_total = 0.0;
...
int cov_evaluate(struct cov_context *context, struct edge_set *new_edges)
{
    ...
    //context->ctx_num_sec_per_edge = context->shmem->num_sec_per_edge;

    context->ctx_num_sec_cov_total = context->shmem->num_sec_cov_total;
    //context->ctx_num_sec_cov_total_double += context->ctx_num_sec_cov_total;

    //context->ctx_num_sec_per_edge_total = context->shmem->num_sec_per_edge_total;
    //context->ctx_num_sec_per_edge_total_double += context->ctx_num_sec_per_edge_total;

    //char buffer[50];
    //int length1, length2, length3;
    // Calcula o número de caracteres de cada valor convertendo-o em string
    //length1 = snprintf(buffer, sizeof(buffer), "%d", context->ctx_num_sec_cov_total);
    //length2 = snprintf(buffer, sizeof(buffer), "%d", context->shmem->num_sec_cov_total);
    //length3 = snprintf(buffer, sizeof(buffer), "%.0f", context->ctx_num_sec_cov_total_double);

    // Imprimindo os valores e o número de caracteres
    //printf("ctx_num_sec_cov_total: %d[%d] num_sec_cov_total: %d[%d], ctx_num_sec_cov_total_double: %f[%d]\n",
    //       context->ctx_num_sec_cov_total, length1,
    //       context->shmem->num_sec_cov_total, length2,
    //       context->ctx_num_sec_cov_total_double, length3);
    //if ((context->shmem->num_sec_cov > 0))
    

    if ((context->shmem->num_sec_cov > context->ctx_num_sec_cov) && (num_new_edges > 0))
    {
        printf("[JST] FoundSecCov\n");
        context->ctx_num_sec_cov = context->shmem->num_sec_cov;
        context->found_sec_cov = true;
    }
    else
    {
        context->found_sec_cov = false;
    }
