...
if let weight1 = args["--program-weight1"], let parsedWeight1 = Double(weight1) {
    programWeight1 = parsedWeight1
    print("[Config] Program weight1 set to \(programWeight1)")
} else {
    print("[Config] Using default program weight1: \(programWeight1)")
}

if let weight2 = args["--program-weight2"], let parsedWeight2 = Double(weight2) {
    programWeight2 = parsedWeight2
    print("[Config] Program weight2 set to \(programWeight2)")
} else {
    print("[Config] Using default program weight2: \(programWeight2)")
}

if let weight3 = args["--program-weight3"], let parsedWeight3 = Double(weight3) {
    programWeight3 = parsedWeight3
    print("[Config] Program weight3 set to \(programWeight3)")
} else {
    print("[Config] Using default program weight3: \(programWeight3)")
}


// Verifying the arguments for generator selection
var generatorArguments = [String]()

if args["--use-loop-condition"] != nil {
    generatorArguments.append("--use-loop-condition")
}
if args["--use-math"] != nil {
    generatorArguments.append("--use-math")
}
if args["--use-read-write"] != nil {
    generatorArguments.append("--use-read-write")
}
if args["--use-functions"] != nil {
    generatorArguments.append("--use-functions")
}
if args["--use-object-array"] != nil {
    generatorArguments.append("--use-object-array")
}


// Selects generators based on the provided arguments
var selectedGenerators = chooseGenerators(from: generatorArguments)

// Function to select generators based on arguments
func chooseGenerators(from arguments: [String]) -> [CodeGenerator] {
    var selectedGenerators = [CodeGenerator]()
    
    // If no arguments are provided, use all generators by default
    if arguments.isEmpty {
        return essentialGenerators + CodeGenerators
    }
    
    // Check which groups were requested and add the corresponding generators
    if arguments.contains("--use-loop-condition") {
        selectedGenerators.append(contentsOf: loopAndConditionGenerators)
    }
    if arguments.contains("--use-math") {
        selectedGenerators.append(contentsOf: mathGenerators)
    }
    if arguments.contains("--use-read-write") {
        selectedGenerators.append(contentsOf: readWriteGenerators)
    }
    if arguments.contains("--use-functions") {
        selectedGenerators.append(contentsOf: functionGenerators)
    }
    if arguments.contains("--use-object-array") {
        selectedGenerators.append(contentsOf: objectArrayGenerators)
    }
    
    // Always include the essential generators
    selectedGenerators.append(contentsOf: essentialGenerators)
    
    // If no specific group was selected, return all generators, including the essential ones
    return selectedGenerators.isEmpty ? essentialGenerators + CodeGenerators : selectedGenerators
}

var mutatorArguments = [String]()

let availableMutatorsWithWeights: [String: (Mutator, Int)] = [
    "exploration": (ExplorationMutator(), 3),
    "codegen": (CodeGenMutator(), 2),
    "splice": (SpliceMutator(), 2),
    "probing": (ProbingMutator(), 2),
    "input-naive": (InputMutator(isTypeAware: false), 2),
    "input-aware": (InputMutator(isTypeAware: true), 1),
        // Can be enabled for experimental use, ConcatMutator is a limited version of CombineMutator
    // "concat": (ConcatMutator(), 1) 
    "operation": (OperationMutator(), 1),
    "combine": (CombineMutator(), 1),
    // Include this once it does more than just remove unneeded try-catch
    // "fixup": (FixupMutator(), 1),
     "complexstruct": (ComplexStructMutator(), 1),
    //  "oob": (OOBMutator(), 1),
    
]

// Verifying  mutators were specified
if let specifiedMutators = args["--mutators"] {
    mutatorArguments = specifiedMutators.split(separator: ",").map { String($0) }
}

// Select mutators based on the provided arguments
let selectedMutators = chooseMutators(from: mutatorArguments)

func chooseMutators(from arguments: [String]) -> [(Mutator, Int)] {
    var selectedMutators = [(Mutator, Int)]()
    
    // If no argument was provided, use all mutators with their weights
    if arguments.isEmpty {
        return Array(availableMutatorsWithWeights.values)
    }
    
    // Add the mutators specified in the arguments
    for arg in arguments {
        if let mutatorWithWeight = availableMutatorsWithWeights[arg] {
            selectedMutators.append(mutatorWithWeight)
        } else {
            print("Invalid mutator: \(arg)")
        }
    }
    
    // If no valid mutator was found, return all by default
    return selectedMutators.isEmpty ? Array(availableMutatorsWithWeights.values) : selectedMutators
}
...
Options:
    //JST
    --use-loop-condition          : Use generators that focus on loop and conditional structures.
    --use-math                    : Use generators that perform math operations.
    --use-read-write              : Use generators that handle read and write operations.
    --use-functions               : Use generators that deal with function operations.
    --use-object-array            : Use generators for objects and arrays.
    --mutators=exploration,codegen : Use only the specified mutators for fuzzing.
...
/// JST
///
// For fuzzili  replace selectedGenerators.map to CodeGenerators.map
let regularCodeGenerators: [(CodeGenerator, Int)] = selectedGenerators.map {
    ...
    // Adicione o print para verificar o mapeamento
    print("Code Generator: \($0.name), Weight: \(weight)")
    return ($0, weight)
}
...
func makeFuzzer(with configuration: Configuration) -> Fuzzer {
    ...
    let mutators = WeightedList(selectedMutators)
