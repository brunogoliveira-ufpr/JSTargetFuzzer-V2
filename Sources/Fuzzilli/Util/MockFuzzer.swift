...
class MockEvaluator: ProgramEvaluator {
    ...
    init() {
        self.logger = Logger(withLabel: "MockEvaluator")
    }

    func evaluate(_ execution: Execution) -> ProgramAspects? {
        // Simula a lógica para definir `covered` baseado na execução
        if execution.outcome == .succeeded {
            covered = Bool.random()  // Aleatoriamente decide se há cobertura para simulação
            logger.warning("[MockEvaluator] Coverage evaluation (succeeded): \(covered)")
        } else {
            covered = false
            logger.warning("[MockEvaluator] Coverage evaluation (failed): \(covered)")
        }
        return nil
    }
    ...
    var numSecCov: Double {
        return 50.0 
    }

    var numSecCovTotal: Double {
        return 0 
    }
    
    var numSecPerEdge: Double {
        return 0.5 
    }
    var numSecPerEdgeTotal: Double {
        return 0 
    }
    ...
    // Implementação do protocolo Component
    func initialize(with fuzzer: Fuzzer) {}
